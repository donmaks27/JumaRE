...
namespace JumaRenderEngine
{
    ...
    bool VulkanCommandPool::init(const VulkanQueueType queueType, const VkCommandPoolCreateFlags flags)
    {
        ...
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan command pool for queue family {}"), queueDescription->familyIndex);
            return false;
        }
    ...
    bool VulkanCommandPool::createCommandBuffer(const bool primaryLevel, VulkanCommandBuffer& outCommandBuffers)
    {
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to allocate command buffer"));
            return false;
        }
        return true;
    }
