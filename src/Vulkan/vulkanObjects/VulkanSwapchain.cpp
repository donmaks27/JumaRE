...
namespace JumaRenderEngine
{
    ...
    bool VulkanSwapchain::init(const window_id windowID)
    {
        ...
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create RenderAvailableSemaphore"));
            clearVulkan();
            return false;
        }
    ...
    bool VulkanSwapchain::createSwapchain(VkSwapchainKHR oldSwapchain)
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to get window {}"), m_WindowID);
            return false;
        }
        ...
        {
            JUTILS_LOG(error, JSTR("There is no vulkan surface formats for window {}"), m_WindowID);
            return false;
        }
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan swapchain"));
            return false;
        }
    ...
    bool VulkanSwapchain::acquireNextImage(bool& availableForRender)
    {
        ...
        if ((result != VK_SUCCESS) && (result != VK_SUBOPTIMAL_KHR))
        {
            ...
            {
                JUTILS_ERROR_LOG(result, JSTR("Failed to acquire swapchain image"));
                return false;
            }
    ...
    void VulkanSwapchain::onWindowPropertiesChanged(WindowController* windowController, const WindowData* windowData)
    {
        ...
        {
            if (windowData->size != m_SwapchainImagesSize)
            {
                invalidate();
            }
        }
    ...
    bool VulkanSwapchain::update()
    {
        if (m_SwapchainInvalid)
        {
            ...
            {
                JUTILS_LOG(error, JSTR("Failed to recreate vulkan swapchain"));
                return false;
            }
