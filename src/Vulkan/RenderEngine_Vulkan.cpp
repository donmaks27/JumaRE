...
namespace JumaRenderEngine
{
    ...
    bool RenderEngine_Vulkan::initInternal(const jmap<window_id, WindowProperties>& windows)
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to pick physical device"));
            return false;
        }
        if (!createDevice())
        {
            JUTILS_LOG(error, JSTR("Failed to create vulkan device"));
            return false;
        }
        if (!createCommandPools())
        {
            JUTILS_LOG(error, JSTR("Failed to create command pools"));
            return false;
        }
        if (!getWindowController<WindowController_Vulkan>()->createWindowSwapchains())
        {
            JUTILS_LOG(error, JSTR("Failed to create vulkan swapchains"));
            return false;
        }
        return true;
    }
    ...
    bool RenderEngine_Vulkan::createVulkanInstance()
    {
        ...
        const char* validationLayers[validationLayerCount] = { "VK_LAYER_KHRONOS_validation" };
        {
            ...
            if (!layersFounded)
            {
                JUTILS_LOG(error, JSTR("Failed to find validation layers"));
                return false;
            }
        }
        ...
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan instance"));
            return false;
        }
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create debug messenger"));
            return false;
        }
#endif
        return true;
        ...
        VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData)
    {
        ...
        {
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT: JUTILS_LOG(warning, pCallbackData->pMessage); break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT: JUTILS_LOG(error, pCallbackData->pMessage); break;
        default: ;
        }
        return VK_FALSE;
    }
    ...
    bool RenderEngine_Vulkan::createDevice()
    {
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan device"));
            return false;
        }
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create VMA allocator"));
            return false;
        }
        return true;
    }
    ...
    bool RenderEngine_Vulkan::createCommandPools()
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create graphics command pool"));
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create transfer command pool"));
    ...
    VkSampler RenderEngine_Vulkan::getTextureSampler(const TextureSamplerType samplerType)
    {
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan sampler"));
            return nullptr;
        }
