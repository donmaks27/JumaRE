...
namespace JumaRenderEngine
{
    ...
    bool RenderPipeline_Vulkan::initInternal()
    {
        ...
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan fence"));
            return false;
        }
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create vulkan semaphore"));
            clearVulkan();
            return false;
        }
    ...
    bool RenderPipeline_Vulkan::onStartRender(RenderOptions* renderOptions)
    {
        ...
        for (const auto& windowID : windowController->getWindowIDs())
        {
            ...
            {
                JUTILS_LOG(error, JSTR("Failed to get data for window {}"), windowID);
                return false;
            }

            if (!swapchain->update())
            {
                JUTILS_LOG(error, JSTR("Failed to update swapchain"));
                return false;
            }
    ...
    bool RenderPipeline_Vulkan::startRecordingRenderCommandBuffer(RenderOptions* renderOptions)
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create render command buffer"));
            return false;
        }
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to start render command buffer record"));
    ...
    bool RenderPipeline_Vulkan::finishRecordingRenderCommandBuffer(RenderOptions* renderOptions)
    {
        ...
        if (result != VK_SUCCESS)
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to finish render command buffer record"));
            commandBuffer->returnToCommandPool();
            return false;
        }
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to submit vulkan render command buffer"));
            commandBuffer->returnToCommandPool();
            return false;
        }
        ...
        if (!m_Swapchains.isEmpty())
        {
            ...
            for (int32 index = 0; index < m_Swapchains.getSize(); index++)
            {
                ...
                {
                    JUTILS_ERROR_LOG(presentResult, JSTR("Failed to present swapchain image"));
                    return false;
                }
            }
        }
