...
namespace JumaRenderEngine
{
    IDXGIAdapter4* GetDXGIAdapter()
    {
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DXGIFactory2"));
            return nullptr;
        }
    ...
    bool RenderEngine_DirectX12::initInternal(const WindowCreateInfo& mainWindowInfo)
    {
#if defined(JDEBUG)
        {
            ...
            if (FAILED(result))
            {
                JUTILS_ERROR_LOG(result, JSTR("Failed to get DirectX12 debug interface"));
                return false;
            }
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create DirectX12 device"));
            return false;
        }
        if (!createCommandQueues())
        {
            JUTILS_LOG(error, JSTR("Failed to create DirectX12 command queues"));
            return false;
        }
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create DirectX12 swapchains"));
            return false;
        }
        return true;
    }
    bool RenderEngine_DirectX12::createDirectXDevice()
    {
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 device"));
            adapter->Release();
            return false;
        }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create D3D12MA resource allocator"));
            device->Release();
            return false;
        }
...
#if defined(JDEBUG)
        {
            ...
            if (SUCCEEDED(device->QueryInterface(&infoQueue)))
            {
                ...
                if (FAILED(result))
                {
                    JUTILS_ERROR_LOG(result, JSTR("Failed to push message filter"));
                    allocator->Release();
                    device->Release();
                    return false;
                }
    ...
    bool RenderEngine_DirectX12::createOtherObjects()
    {
        ...
        m_SamplersDescriptorHeap = createDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, GetTextureSamplerTypeCount(), false);
        D3D12_CPU_DESCRIPTOR_HANDLE descriptor = m_SamplersDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
        for (uint8 samplerID = 0; samplerID < GetTextureSamplerTypeCount(); samplerID++)
        {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to initialize texture mip generator"));
        ...
        const bool shaderVisible) const
    {
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 descriptor heap"));
            return nullptr;
        }
