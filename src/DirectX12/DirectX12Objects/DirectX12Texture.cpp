...
namespace JumaRenderEngine
{
        ...
        const D3D12_RESOURCE_STATES initialState, const D3D12_RESOURCE_FLAGS flags)
    {
        if (isValid())
        {
            JUTILS_LOG(warning, JSTR("DirectX12 texture already initialized"));
            return false;
        }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 texture resource"));
            return false;
        }
    ...
    bool DirectX12Texture::init(ID3D12Resource* existingResource, const D3D12_RESOURCE_STATES currentState)
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Invalid DirectX12 resource object"));
            return false;
        }
    ...
    void DirectX12Texture::setMipLevelsState(jarray<D3D12_RESOURCE_STATES>&& states)
    {
        ...
        {
            JUTILS_LOG(warning, JSTR("Invalid input data"));
        }
    }
