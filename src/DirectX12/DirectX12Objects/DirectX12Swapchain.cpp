...
namespace JumaRenderEngine
{
    ...
    bool DirectX12Swapchain::init(const window_id windowID)
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Can't find window {}"), windowID);
            return false;
        }
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to get command queue"));
            return false;
        }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 swapchain"));
            return false;
        }
        ...
        m_SwapchainBuffersSize = windowData->size;
    ...
    bool DirectX12Swapchain::present()
    {
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to present swapchain image"));
            return false;
        }
    ...
    void DirectX12Swapchain::onWindowPropertiesChanged(WindowController* windowController, const WindowData* windowData)
    {
        ...
        {
            if (windowData->size != m_SwapchainBuffersSize)
            {
                invalidate();
            }
        }
    ...
    bool DirectX12Swapchain::updateSwapchain()
    {
        if (m_SwapchainInvalid)
        {
            ...
            const HRESULT result = m_Swapchain->ResizeBuffers(0, windowData->size.x, windowData->size.y, DXGI_FORMAT_UNKNOWN, swapchainDescription.Flags);
            if (FAILED(result))
            {
            ...
            {
                JUTILS_LOG(error, JSTR("Failed to update window render target"));
                clearDirectX();
                return false;
            }
