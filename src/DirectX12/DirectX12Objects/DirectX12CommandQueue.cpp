...
namespace JumaRenderEngine
{
    ...
    bool DirectX12CommandQueue::init(const D3D12_COMMAND_LIST_TYPE queueType)
    {
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 command queue"));
            return false;
        }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX12 fence"));
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create fence event"));
            fence->Release();
            commandQueue->Release();
            return false;
        }
    ...
    DirectX12CommandList* DirectX12CommandQueue::getCommandList()
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to create command list"));
