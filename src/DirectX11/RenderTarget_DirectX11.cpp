...
namespace JumaRenderEngine
{
    ...
    bool RenderTarget_DirectX11::initInternal()
    {
        ...
        if (isWindowRenderTarget())
        {
            ...
            {
                JUTILS_LOG(error, JSTR("Failed to initialize DirectX11 window render target"));
                return false;
            }
        }
        else if (!initRenderTarget(nullptr))
        {
            JUTILS_LOG(error, JSTR("Failed to initialize DirectX11 render target"));
            return false;
        }
        return true;
    }
    bool RenderTarget_DirectX11::initWindowRenderTarget()
    {
        ...
        {
            JUTILS_LOG(error, JSTR("Failed to get DirectX11 swapchain for window {}"), windowID);
            return false;
        }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to get DirectX11 swapchain image"));
            return false;
        }
    ...
    bool RenderTarget_DirectX11::initRenderTarget(ID3D11Texture2D* resultImage)
    {
        ...
        if (!resolveEnabled)
        {
            ...
            else
            {
                ...
                if (FAILED(result))
                {
                    JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 color image"));
                    return false;
                }
                ...
                if (FAILED(result))
                {
                    JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 color image shader view"));
                    colorImage->Release();
                    return false;
                }
            }
        ...
        if (FAILED(result))
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 color image view"));
        ...
        if (useDepth)
        {
            ...
            if (FAILED(result))
            {
                JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 depth image"));
                if (resultImageView != nullptr)
                {
                    resultImageView->Release();
                }
            ...
            if (FAILED(result))
            {
                JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 depth image view"));
                depthImage->Release();
                if (resultImageView != nullptr)
                {
                    resultImageView->Release();
        ...
        if (resolveEnabled)
        {
            ...
            else
            {
                ...
                if (FAILED(result))
                {
                    JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 resolve image"));
                    if (useDepth)
                    {
        ...
        {
            JUTILS_ERROR_LOG(result, JSTR("Failed to create DirectX11 rasterizer state"));
            resultImageView->Release();
            depthImageView->Release();
            colorImageView->Release();
            resolveImage->Release();
